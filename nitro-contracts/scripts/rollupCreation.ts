...
import { abi as rollupCreatorAbi } from '../build/nitro-contracts/src/rollup/RollupCreator.sol/RollupCreator.json'
...
export async function createRollup(feeToken?: string) {
  ...
  if (!rollupCreatorAbi) {
    throw new Error(
      'You need to first run <deployment.ts> script to deploy and compile the nitro-contracts first'
    )
  }
